    ...
    document.querySelector('.payment-container').style.display = 'block';
        ...
        case "Đặt Hàng":
            ...
            var paymentContainer = document.querySelector('.payment-container');
              ...
              var genderRadioButtons = paymentContainer.querySelectorAll('.infor-customer input[name="inlineRadioOptions"]');
            ...
            var deliveryRadioButtons = paymentContainer.querySelectorAll('.row input[name="deliveryOptions"]');
            ...
            if (selectedDeliveryOption === "momo") {
                ...
                axios.patch(`${apiUrl}/users/${profileData.id}`, {
                    cartItems: []
                 }).then((response) =>{
                    document.querySelector('.payment-container').style.display = 'none';
                    document.querySelector('.cartempty-content').style.display = 'none';
      ...
      productItem.innerHTML = `<div class="row pt-2">
              <div class="col-4 col-md-2">
                  <div class="row">
                      ...
                      <div class="btn-clear col-12" onclick="handleDelete(${product.id})">
                          Xóa
                      </div>
                  </div>
              </div>
                  ...
                  <div class="row">
                      <div class="col-12 col-md-12 pt-2 d-flex align-items-center justify-content-between">
                          ...
                          <div class="input-group">
                              ...
                              <input type="text" value="${product.quantity}" class="product-quantity" id="input-number" name="${product.id}">
                              <div class="input-group-prepend">
...
axios.get(`${apiUrl}/orders`).then((response) => {
    const orders = response.data.filter(order => Number(order.user[0]) === Number(profileData.id))
...
async function handleClick(button, action) {
  if (action === "decrease") {
    ...
    if (quantity > 1) {
        quantity--;
    const productToUpdate = curItems.find(product => parseInt(product.id) === parseInt(input.name));
        if (productToUpdate) {
          productToUpdate.quality = quantity;
          await axios.patch(`${apiUrl}/users/${profileData.id}`, {
            cartItems : curItems
        }).then((response) =>{
