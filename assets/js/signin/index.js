...
async function handleLogin() {
    ...
    await axios.get("https://api-zerot-lowdb.onrender.com/user").then((response) => {
        ...
        if (userExist && userExist.password === password) {
            ...
            if (userExist.status !== "active") {
                ...
                emailjs.send("service_4mv8mgj", "template_69jvbsa", templateParams).then(
                    async function() {
                        await axios.patch(`https://api-zerot-lowdb.onrender.com/user/${userExist.id}`, { code: templateParams.code }).then(res => {
                            setTimeout(() => {
